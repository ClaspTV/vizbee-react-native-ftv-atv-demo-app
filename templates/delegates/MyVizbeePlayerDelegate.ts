/**
 * PlayerDelegate Template
 *
 * Manages video player integration with Vizbee SDK
 * CLIENT TODO: Replace video player implementation
 */
...
export class MyVizbeePlayerDelegate extends VizbeePlayerDelegate {
   ...
   */
  getVideoInfo() {
    const {videoId, title, duration, isLive, imageURL} =
  ...
  getVideoStatus() {
    ...
    } else if (this.videoPlayer?.interrupted) {
    ...
    vizbeeVideoStatus.duration = this.convertSecondsToMilliseconds(
      vizbeeVideoStatus.isPlayingAd
        ? this.videoPlayer?.adDuration
